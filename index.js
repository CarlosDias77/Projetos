...
function cpu() {
    ...
    if (!jogadaVencedora(5)) {
        if (!jogadaVencedora(1)) {
            if (Math.random() < 0.7) {
                if (!jogadaVencedora(5) && !jogadaVencedora(1)) {
                    jogadaAleatoria();
                }
            } else {
                jogadaAleatoria();
...
function jogadaVencedora(jogador) {
    for (let i = 0; i < 8; i++) {
        let soma = 0;
        for (let j of jogo.posvenc[i]) {
            let lin = Math.floor(j / 3);
            let col = j % 3;
            soma += jogo.matriz[lin][col];
        }
        if (soma === 3 * jogador) {
            for (let j of jogo.posvenc[i]) {
                let lin = Math.floor(j / 3);
                ...
                if (jogo.matriz[lin][col] === 0) {
                    som(snd_click, 1, 1.2);
                    let bt = lin * 3 + col;
                    document.getElementById('b' + bt).innerHTML = jogador === 1 ? 'O' : 'X';
                    jogo.matriz[lin][col] = jogador;
                    jogo.rodadas++;
                    jogo.jogador = jogador === 1 ? 2 : 1;
                    fimJogo();
                    return true;
                }
            }
...
function jogadaAleatoria() {
    let lin, col;
    do {
        lin = Math.floor(Math.random() * 3);
        col = Math.floor(Math.random() * 3);
    } while (jogo.matriz[lin][col] !== 0);
    som(snd_click, 1, 1.2);
    let bt = lin * 3 + col;
    document.getElementById('b' + bt).innerHTML = 'O';
